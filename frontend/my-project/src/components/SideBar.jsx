...
const SideBar = () => {
  ...
  const onlineOtherUsers = authUser
    ...
    <aside className='h-full w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200'>
      <div className='border-b border-base-300 w-full p-5'>
        ...
        <div className= "mt-3 hidden lg:flex items-center gap-2">
          ...
          <span className='text-xs text-zinc-500'>({onlineOtherUsers.length} online)</span>
        </div>
      </div>
      ...
      <div className='overflow-y-auto w-full py-3'>
        {filteredUsers.map((user) => (
          <button
            ...
            className={`w-full p-3 flex items-center gap-3 hover:bg-base-300 transition-colors ${selectedUser?._id === user._id ? 
